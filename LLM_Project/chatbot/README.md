...
BPE 토크나이저 → GPT 학습 → RAG → LangChain/LangGraph 파이프라인 → Claude/Qwen 연동 → Gemini 평가까지 전 과정 직접 구현한 한국어 챗봇.
지식 베이스(`ragdata/`)를 **RPG·게임** 도메인으로 채워, 게임 장르·시스템·직군 관련 질문에 특화하여 답변한다.

# 화면 구성도
![화면 구성도](images/chatbot.png)
...
├── docs/                       # 개발 관련 문서 (review.md, instance_info.md, wireshark_report.md)
...
└── source/
    ...
    │   ├── sessions.py         # 사용자별 채팅 세션 생성·조회·이름 변경
...
초기의 단일 `app.py` 중심 구조에서 API, 모델, 검색, 체인, 그래프, 사용자 데이터를 역할별 모듈로 분리했다. 현재 기본 UI의 비교 모드는 동일한 질문을 네 모델의 전용 LangGraph에 병렬로 전달하고, 모든 답변이 완료되면 Gemini가 최선 답변을 선택한다.

![4개 모델 비교 실행 구조](images/compare_execution_flow.png)

네 그래프는 동일한 BM25+FAISS 하이브리드 검색기를 사용하며 관련성이 부족하면 임계값을 낮춰 최대 2회 재시도한다. RAG 성공 시 SOP_GPT는 Span 모델로 정답 구간을 추출하고, Claude와 Qwen은 검색 문서를 컨텍스트로 답변을 생성한다. 재시도 후에도 검색 기준을 충족하지 못하면 각 모델이 직접 답변한다.
...
웹 UI
  └─ FastAPI (`source/app`)
      ...
      ├─ 일반 응답 / SSE 스트리밍 라우터
      └─ 요청 모드 선택
          ├─ Basic ──────────────── SOP_GPT / Claude
          ├─ TF-IDF RAG ────────── 검색 → Span 추출 또는 직접 응답
          ├─ LangChain ──────────── BM25+FAISS → LCEL 체인
          ├─ LangGraph ──────────── 검색 → 판정 → 재시도 → 응답
          │   ├─ SOP_GPT
          │   ├─ Claude
          │   ├─ Qwen BF16
          │   └─ Qwen Q4_K_M
          ├─ Auto ───────────────── 질문 분류 → 적합한 체인 선택
          └─ Compare ────────────── 4개 LangGraph 병렬 실행 → Gemini Judge

공통 계층
  ├─ `source/llm`   모델별 추론·스트리밍 래퍼
  ├─ `source/lc`    하이브리드 검색기와 LangChain 체인
  ├─ `source/lg`    LangGraph 상태·노드·그래프
  ├─ `source/rag`   TF-IDF 검색과 KorQuAD 처리
  ├─ `source/model` SOP_GPT 학습·체크포인트와 Qwen 모델
  └─ `source/data`  사용자·세션·대화 기록 영속화
```
...
| `PATCH /sessions/{session_id}` | 세션 이름 변경 |
| `POST /chat/compare/stream` | **4모델 병렬 비교** SSE 스트림 (추천) |
| `POST /chat/compare/judge/stream` | 세션 종합 평가 (Gemini Judge) SSE 스트림 |
| `GET /chat/compare/history` | 비교 세션 대화 기록 조회 |
| `POST /chat/auto/stream` | 자동 라우팅 SOP_GPT 스트리밍 |
| `POST /chat/claude/auto/stream` | 자동 라우팅 Claude 스트리밍 |
| `POST /chat/{basic\|rag\|langchain\|langgraph}` | SOP_GPT 각 모드 |
| `POST /chat/claude/{basic\|rag\|langchain\|langgraph}` | Claude 각 모드 |
| `POST /chat/langgraph/stream` | `model` 값(`sop`, `claude`, `qwen`, `qwen-q`)에 따른 LangGraph 스트리밍 |
| `GET /` | 웹 UI |
| `GET /docs` | Swagger UI |

SSE 이벤트 타입: `model_text` · `model_done` · `judge_text` · `judge_done` · `mode` · `text` · `text_fallback` · `rag_context` · `status` · `done`
