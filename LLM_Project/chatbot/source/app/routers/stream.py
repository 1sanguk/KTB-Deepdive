...
from streaming import _sse, sop_stream, sop_rag_stream, claude_rag_stream

router = APIRouter()


@router.post("/chat/basic/stream")
async def chat_basic_stream(req: ChatRequest):
...
async def chat_langchain_stream(req: ChatRequest):
    return StreamingResponse(
        sop_rag_stream(
...
async def chat_claude_basic_stream(req: ChatRequest):
    async def gen():
...
async def chat_claude_rag_stream(req: ChatRequest):
    return StreamingResponse(
...
async def chat_claude_langchain_stream(req: ChatRequest):
    return StreamingResponse(
