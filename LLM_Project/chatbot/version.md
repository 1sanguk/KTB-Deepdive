...
## 2026-06-14 — 최초 프로젝트 (5주차 위클리 챌린지)
<detail>
<summary><b>한국어 Mini-GPT 챗봇 최초 구현.</b></summary>
...
## 2026-06-20 — RAG(검색 증강 생성) 아키텍처 적용 (개인 프로젝트)
<detail>
<summary><b>5주차 챗봇에 RAG를 적용해 검색 결과를 실제로 활용하는 Q&A로 발전.</b></summary>
...
- **Stage 3 파인튜닝 추가**: `SOP_GPT_qa.pt`에서 이어서 KorQuAD (참고/질문/답변) 트리플로 한 번 더 파인튜닝한 `SOP_GPT_rag.pt` 도입
  - 단순히 검색 결과를 프롬프트에 끼워넣는 것이 아니라, 모델이 `"참고: ..."` 필드를 실제로 읽고 답변에 반영하도록 학습시킴
...
- **검증 결과**: 검색(retrieval)은 질문과 관련된 KorQuAD 문단을 정확히 찾아내는 것을 확인. 다만 답변 생성 정확도는 모델 규모(256dim/6layer/8000vocab) 한계로 사실 추출형 질문에서는 부정확한 경우가 있었고, 잡담형 질문에서는 관련 없는 문서가 검색되어 오히려 Stage 2보다 부자연스러운 답변이 나오는 경우도 있음을 확인 — 향후 유사도 임계값 기반 폴백(관련 문서가 없으면 Stage 2 모델로 대체) 등 개선 여지로 남김
</detail>

## 2026-06-20 (이어서) — 생성형 RAG를 추출형(extractive) QA로 교체 + 유사도 임계값 폴백
<detail>
<summary><b>위 Stage 3(생성형 RAG)의 한계(부정확한 답, 잡담형 질문 역효과)를 해결하기 위해 구조를 변경.</b></summary>
- **디코딩 파라미터 튜닝**: `model.py`의 `generate()`에 top-p(nucleus) 샘플링 추가, Stage1 이어쓰기 기본값을 `temperature=0.8, top_k=40` → `temperature=0.7, top_p=0.9`로 변경 — 문장이 짧아지고 깨진 인용부호 등 잡음은 줄었지만 의미적 개선은 제한적
...
- **유사도 임계값 기반 폴백 라우팅**: `/chat`에서 검색 유사도가 0.25 이상이면 Stage4(추출형) 사용, 그보다 낮으면 Stage2(잡담형) 모델로 자동 전환 — "오늘 기분은 어때?", "사랑해" 같은 질문이 더는 무관한 위키 문서를 끼워 넣지 않고 자연스럽게 답하도록 개선, 응답에 `used_rag` 필드로 어느 경로를 탔는지 노출
- 안 쓰게 된 `SOP_GPT_rag.pt`(Stage3 생성형 체크포인트) 삭제
</detail>

## 2026-06-20 (이어서 2) — TF-IDF 단독 검색을 임베딩 결합 하이브리드 검색으로 교체
<detail>
<summary><b>임계값 라우팅의 오분류(잡담 질문이 RAG로 새는 경우)를 줄이기 위해 검색기를 개선.</b></summary>
- **임베딩 검색 시도**: `transformers`의 `klue/bert-base`로 1차 시도 — 문장 유사도용으로 학습된 모델이 아니라 모든 문장이 비슷한 점수로 뭉치는 anisotropy 문제 확인, 문장 유사도(STS)로 파인튜닝된 `jhgan/ko-sroberta-multitask`로 교체 (실제 SKT KoBERT는 별도 sentencepiece 토크나이저 패키지가 필요해 AutoTokenizer로 바로 쓸 수 있는 동급 모델로 대체)
...
## 회고
<detail>
  <summary><b>2026.06 6주차 수업의 봇 프로젝트 개선</b></summary>
