...
## 2026-07-07 — 히스토리 누적 버그 수정 + 최소 생성 길이 보장

### 히스토리 누적 버그 수정 (`source/app/streaming.py`)

**버그**: `with_history`에서 `answer += data.get("text", "")` 로 누적하고 있었는데, `stream_tokens`가 매 토큰마다 전체 누적 텍스트를 보내는 구조라 히스토리에 `"잘잘하는잘하는게잘하는게요.잘하는게요...."` 형태로 저장되는 현상.

**원인**: `stream_tokens`는 delta가 아니라 누적 텍스트를 SSE로 전송한다. `+=`로 이어붙이면 n번째 이벤트까지의 텍스트가 n번 중복 합산된다.

**수정**: `+=` → `=` 로 변경. 마지막 `text` 이벤트가 완성된 전체 텍스트이므로 덮어쓰기만 하면 된다.
...
`stop_on="sentence"` 상태에서 모델이 첫 토큰부터 `"네?"` 같은 단어를 생성하면 즉시 멈춰 1~2 단어 응답이 출력되는 문제. `min_new_tokens` 파라미터를 추가해 일정 토큰 수 이전에는 stop token을 무시한다.

**변경 파일:**
- `source/model/model.py` — `generate`, `generate_stream`에 `min_new_tokens=0` 파라미터 추가
- `source/lc/llm.py` — `SOP_GPT_LLM`에 `min_new_tokens: int = 0` 필드 추가
- `source/app/state.py` — `qa_llm`에 `min_new_tokens=20` 적용

---

### UI status 이벤트 처리 (`source/app/ui.py`)

LangGraph 검색 시 `"검색 중...."` status 텍스트를 버블에 표시하고, 이후 `text` 이벤트가 오면 같은 버블에 덮어쓰는 방식 유지. (이전 세션에서 실수로 제거했다가 복구)
